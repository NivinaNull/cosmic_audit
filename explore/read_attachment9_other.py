...
# @File    : read_attachment9_other.py
...
def get_cosmic_info(cosmic_sheet):
    ...
    part1 = cosmic_sheet[cosmic_sheet.iloc[:, 0].str.strip() == '需求序号'].reset_index()
    ...
    #part1.drop(columns='index').dropna(axis=1)  ###去重空字段
    if part1.empty:
    ...
    if part2.empty:
        return False, 'ERROR: 《表2 需求开发工作量核算表》 没有找到需求提出人相关行'
    part2.dropna(inplace=True, axis=1)
    b3 = (part2.iloc[:, 1].astype(str).str.strip() == '').any()
    if part2.shape[0] == requirement_count and part2.shape[1] == 6 and not b3:
        temp_info['advocator'] = part2.iloc[:, 1].astype(str)
    else:
    ...
    coding_requirement_indices = coding_requirement_indices.append(coding_requirement_indices + 1).append(coding_requirement_indices + 2)
    coding_requirement = cosmic_sheet.iloc[coding_requirement_indices, :]

    # 找到代码的功能点描述
    # print('coding_requirement_before '* 5 + '\n', coding_requirement)
    # for j in coding_requirement.columns:
    #     print('type(j)'*10 + '\n', type(j))
    #     <class 'int'>
    ...
    # print('contrast ' * 20 + '\n', coding_requirement.iloc[:, 1], '\n', coding_requirement.loc(axis=1)[2])
    coding_requirement = coding_requirement.astype(str)
    coding_requirement = coding_requirement.loc[(coding_requirement.iloc[:, 1].str.contains('功能')) & (coding_requirement.iloc[:, 1].str.contains('列表'))]
    if coding_requirement.shape[0] != len(coding_requirement_indices) / 3:
        print('功能点名称列表 doesnt match 代码开发' * 2 + '\n', file_path, '\n', '代码开发->功能点名称 相关行数：', coding_requirement.shape[0], '代码开发相关行数：', int(len(coding_requirement_indices) / 3))
    coding_requirement.iloc[:, 2].fillna('无', inplace=True)
    coding_requirement = coding_requirement.dropna(axis=1)
    # print(coding_requirement)
    # print(coding_requirement.columns)
    ...
    if not coding_requirement.empty and len(coding_requirement.columns) == 2:
        ...
        # print('coding_requirement_after_reset' * 5 + '\n', coding_requirement)
        for r in temp_info['index_stride']:
            ...
            if value and (value[0].strip() != '无') and (value[0].strip() != '不涉及'):
    ...
    if temp_info.coding_requirement.hasnans:
        ...
        print('发现没有<代码描述>的cosmic需求，序号：',suspicious_req.values, '\n',  file_path)
...
def check_nonfiles(noncosmic_result):
    for f in filenames:
        if ('非COSMIC' in f or '非COSMIC'.lower() in f) and '__MACOSX' not in folderName:

            if platform.system() == 'Windows':
                local_path = folderName + '\\' + f
...
if platform.system() == 'Windows':
    path = 'C:\\ChinaMobile\\cosmic_files\\专家评审材料_2020Q3'
elif platform.system() == 'Darwin':
    path = './cosmic_files'
else:
    path = './cosmic_files'

# path = 'C:\\ChinaMobile\\cosmic_files'
# path = 'C:\\ChinaMobile\\cosmic_files\\专家评审材料_2020Q1\\201913276\\2020-1-201913276- 云化统一信息平台（核心业务）需求变更支持项目服务'
file_count = 0
sketch_have_read = 0
cosmic_have_read = 0
...
# print('over_ here ' * 8 + '\n', cosmic_info[cosmic_info.project_name == '一级IOP'])
print('over_ here ' * 8 + '\n', cosmic_info)
